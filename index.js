...
function remap(objectArray) {
  return objectArray.map(
    function (x) {
      let obj = {'day':x.day,'WFH':0,'ONSITE':0,'REM':0,'TRAVEL':0,'SKEL':0,'OFF':0}
      if(x.timeType!="NA"){
        obj[x.timeType]=x.time;
        obj.SKEL=x.time_pause_deducted;
        return obj;
      }
    }
  )
...
import('./pkg')
  .then(rust_module => {
    ...
    reader.onload = function(ev) {  
      ...
      console.log(times);
      let wfh_times = [],onsite_times=[],travel_times=[],skel_times=[];
      let old_xd = "";
      times.reverse().forEach(x => {
                if(x && x.day!='na'){
                  ...
                  details.insertAdjacentHTML(
                    ...
                    "<div class='table-cell'>"+x.TRAVEL+"</div>"+
                    "<div class='table-cell'>"+x.SKEL+"</div></div>",
                  );
                  wfh_times.push(x.WFH);
                  onsite_times.push(onsite);   
                  travel_times.push(x.TRAVEL);  
                  skel_times.push(x.SKEL);
                  old_xd = x.day;  
                }

        });
        console.log(skel_times);
        ...
        sessionStorage.setItem("skel_times",skel_times.reverse().join("\n"));
    }
